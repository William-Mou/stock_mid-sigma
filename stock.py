...
def getstock(stocknumber):
    ...
    format_time = "%s%s%s" % (data_time.year, data_time.month, data_time.day)
    url = 'http://www.twse.com.tw/exchangeReport/STOCK_DAY?response=json&date=' + format_time + '&stockNo=' + stocknumber
    print(url)
...
def Standard_Deviation(stocknumber):
    ...
    if len(stocklist) != 0:
        ...
        elif pd.to_numeric(stockdf['收盤價'][-1:]).values[0] > stockAverage + (2*stockSTD):
            buy = '很貴不要買！'
...
Standard_Deviation(stocknumber='2337')